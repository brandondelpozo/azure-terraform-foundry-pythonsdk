...
flowchart TD
    ...
    %% ─── Step 2: Client uploads file directly to Blob Storage ──────────
...
| **HTTP Trigger** `/generate-upload-url` | Azure Function | Generates a write-only SAS URL (15 min TTL) so the client can upload `.txt`/`.docx` files directly to Blob Storage |
| **Blob Trigger** `uploads/{name}` | Azure Function | Fires automatically when a file lands in the `uploads` container; extracts text, runs the 3-agent pipeline, and saves `_improved.docx` + `.metadata.json` to `results/` |
| **`uploads` container** | Azure Blob Storage | Receives client-uploaded files via SAS URL |
| **`results` container** | Azure Blob Storage | Stores the pipeline output: improved `.docx` and `.metadata.json` per processed file |
